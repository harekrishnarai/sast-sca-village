      ...
      <div className="relative z-10">
        ...
        <main className="container mx-auto px-4 py-12">
          ...
          <p className="text-muted-foreground text-lg text-center max-w-3xl mx-auto mb-12">
            The experts behind the SAST SCA Village, bringing together years of experience 
            in application security, static analysis, and software supply chain security.
          </p>

          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
            {teamMembers.map((member) => (
              <div key={member.name} className="bg-card/50 backdrop-blur-sm p-8 rounded-lg border border-border hover:border-accent/50 transition-all group relative overflow-hidden">
                ...
                <div className="absolute inset-0 grid grid-cols-8 grid-rows-8 gap-px pointer-events-none">
                  {Array.from({ length: 64 }).map((_, i) => (
                    <div key={i} className="border border-accent/5 opacity-0 group-hover:opacity-100 transition-all" style={{ transitionDelay: `${i * 10}ms` }}></div>
                  ))}
                </div>
                ...
                <div className="relative">
                  <h3 className="text-xl font-bold text-center mb-1 group-hover:text-accent transition-colors">{member.name}</h3>
                  <p className="text-accent/80 font-mono text-sm text-center mb-2">{member.alias}</p>
                  <p className="text-accent font-medium text-center mb-3">{member.role}</p>
                  ...
                  <div className="flex justify-center gap-4 mt-4">
                    {member.socials.github && (
                      ...
                      >
                        ...
                        <span className="absolute -top-8 left-1/2 -translate-x-1/2 bg-accent/90 text-white text-xs px-2 py-1 rounded opacity-0 group-hover/icon:opacity-100 transition-opacity">GitHub</span>
                      </a>
                    )}
                    {member.socials.twitter && (
                      ...
                      >
                        ...
                        <span className="absolute -top-8 left-1/2 -translate-x-1/2 bg-accent/90 text-white text-xs px-2 py-1 rounded opacity-0 group-hover/icon:opacity-100 transition-opacity">Twitter</span>
                      </a>
                    )}
                    {member.socials.linkedin && (
                      ...
                      >
                        ...
                        <span className="absolute -top-8 left-1/2 -translate-x-1/2 bg-accent/90 text-white text-xs px-2 py-1 rounded opacity-0 group-hover/icon:opacity-100 transition-opacity">LinkedIn</span>
                      </a>
                    )}
                  </div>
                </div>
